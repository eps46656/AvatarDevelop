...
def build_rotation(r):
    ...
    R = torch.zeros((q.size(0), 3, 3), dtype=utils.FLOAT, device=utils.DEVICE)
...
def build_scaling_rotation(s, r):
    L = torch.zeros((s.shape[0], 3, 3), dtype=utils.FLOAT, device=utils.DEVICE)
    R = build_rotation(r)
...
def strip_lowerdiag(L):
    uncertainty = torch.zeros(
        (L.shape[0], 6), dtype=utils.FLOAT, device=utils.DEVICE)
    uncertainty[:, 0] = L[:, 0, 0]
    uncertainty[:, 1] = L[:, 0, 1]
    uncertainty[:, 2] = L[:, 0, 2]
    uncertainty[:, 3] = L[:, 1, 1]
...
class GaussRenderer(nn.Module):
    ...
    def render(self, camera, means2D, cov2d, color, opacity, depths):
        ...
        pix_coord = torch.stack(torch.meshgrid(
            ...
            indexing='xy'
        ), dim=-1).to(device=utils.DEVICE)

        self.render_color = torch.ones(
            *pix_coord.shape[:2], 3).to(device=utils.DEVICE)
        self.render_depth = torch.zeros(
            *pix_coord.shape[:2], 1).to(device=utils.DEVICE)
