...
@beartype
def scatter(
    val_buffer: torch.Tensor,  # [H, W, C]
    weight_buffer: torch.Tensor,  # [H, W]

    point_pos: torch.Tensor,  # [N, 2]
    point_val: torch.Tensor,  # [N, C]
    ...
    point_idx = torch.empty_like(point_pos, dtype=torch.long)

    torch.floor(point_pos, out=point_idx)

    point_remaider = point_pos - point_idx

    radius = std * 3

    offsets_list = list(
        rendering_utils.rasterize_ellipse_with_coeff(
            c_x2=1 / radius**2,
            c_xy=0,
            c_y2=1 / radius**2,
            c_x=0,
    ...
    offsets = torch.tensor(
        offsets_list,
        dtype=torch.long, device=point_pos.device)
    # [K, 2]

    K = offsets.shape[0]

    for offset_i in range(K):
        offset = offsets_list[offset_i]
        # [int, int]

        dist = utils.vec_norm(offsets[offset_i, :] - point_remaider)
        # [N]

        cur_weight = 5
        # TODO

        val_buffer[padding_h - offset[0]:, padding_w - offset[1], :] += \
            point_val * cur_weight

        weight_buffer[padding_h - offset[0]:, padding_w - offset[1], :] += \
            cur_weight
